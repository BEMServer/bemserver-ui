...
class TestCommonTime:
    ...
    # @pytest.mark.parametrize("tz", [None, dt.timezone.utc, ZoneInfo("Europe/Paris")])
    @pytest.mark.parametrize("tz", [ZoneInfo("Europe/Paris")])
    def test_get_weekend_timestamps_range(self, tz):
        ...
        weekend_timestamps = get_weekend_timestamps_range(dt_start, dt_end)

        assert len(weekend_timestamps) == 9
        for weekend in weekend_timestamps:
            assert len(weekend) == 2

        expected_first_weekend = [
            dt.datetime(2025, 1, 4, tzinfo=tz),
            dt.datetime(2025, 1, 5, tzinfo=tz),
        ]
        assert weekend_timestamps[0] == expected_first_weekend
        ...
        assert len(weekend_timestamps) == 5
        for weekend in weekend_timestamps:
            assert len(weekend) == 2
        ...
        assert len(weekend_timestamps) == 1
        for weekend in weekend_timestamps:
            assert len(weekend) == 2

        expected_first_weekend = [
            dt.datetime(2025, 4, 12, tzinfo=tz),
            dt.datetime(2025, 4, 13, tzinfo=tz),
        ]
        assert weekend_timestamps[0] == expected_first_weekend
