...
def retrieve_registered():
    ...
    registered_tasks = {
        task_type: {
            task["name"]: {
                task_param: (
                    json.dumps(task_param_value)
                    if task_param == "default_parameters"
                    else task_param_value
                )
                for task_param, task_param_value in task.items()
            }
            for task in tasks
        }
        for task_type, tasks in tasks_resp.data.items()
    }
    return flask.jsonify(registered_tasks)
...
def _prepare_task_data(task):
    task["task_state"] = "enabled" if task["is_enabled"] else "disabled"
    task["parameters"] = json.dumps(task["parameters"])
    task["schedule_offset"] = "; ".join(
        [str(task["start_offset"]), str(task["end_offset"]), task["offset_unit"]]
    )
    return task
