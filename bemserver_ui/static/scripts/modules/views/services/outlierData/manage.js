        ...
        if (this.#campaignServiceIDInputElmt.value == "") {
            if (isEnabled) {
                ...
                this.#updateStateReqID = this.#internalAPIRequester.post(
                    flaskES6.urlFor(`api.services.outlier_data.enable`),
                    ...
                    (data) => {
                        ...
                        let flashMsgElmt = new FlashMessage({ type: FlashMessageTypes.INFO, text: "Check outlier data service enabled!", isDismissible: true });
            ...
            this.#updateStateReqID = this.#internalAPIRequester.put(
                flaskES6.urlFor(`api.services.outlier_data.update_state`, {id: this.#campaignServiceIDInputElmt.value}),
                { is_enabled: isEnabled },
                this.#etagInputElmt.value,
                (data) => {
                    this.#etagInputElmt.value = data.etag;

                    let flashMsgElmt = new FlashMessage({ type: FlashMessageTypes.INFO, text: `Check outlier data service ${isEnabled ? "en": "dis"}abled!`, isDismissible: true });
                    this.#messagesElmt.appendChild(flashMsgElmt);
                },
                (error) => {
                    let flashMsgElmt = new FlashMessage({ type: FlashMessageTypes.ERROR, text: error.toString(), isDismissible: true });
